...
acan_map <- tm_shape(world, bbox = sbbox) + 
  ...
  tm_dots(col = "species", size = .3, shape = 21) +
...
#tectarchus
tect_points <- subset(coord_points, genus == "tectarchus")
tect_points$species <- droplevels(tect_points$species)
tect_map <- tm_shape(world, bbox = sbbox) + 
  tm_polygons() + 
  tm_shape(tect_points) + 
  tm_dots(col = "species", size = .3, shape = 21) +
  tm_layout(legend.position = c("LEFT", "TOP"),  title = tect_points$genus)


#combo plot of each individual genus
tmap_arrange(all_map, acan_map, argo_map, clit_map, nive_map, tect_map, ncol = 3, nrow = 2)

#generic code to write to svg file (insert which map to write as svg where indicated). Give a unique name and include the folder you want to save it to (if not the working directory). 
svg(file = "maps/combo-map_6-26-18.svg", bg = "transparent")
tmap_arrange(all_map, acan_map, argo_map, clit_map, nive_map, tect_map, ncol = 3, nrow = 2)
dev.off()
